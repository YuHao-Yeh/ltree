...
def print_stats(node: TreeNode, config: TreeConfig, fmt: str = "text") -> None:
    ...
    if config.show_size:
        if fmt == "rich":
            ...
            size_str = (
                f" ({format_size_classic(node.size, config.human_readable).strip()})"
            )
    ...
    if fmt == "rich":
        ...
        console.print(
            f"  Visible: [bold cyan]{s.visible_dirs:>3}[/] directories, [bold cyan]{s.visible_files:>3}[/] files"
        )
        console.print(
            f"  Total  : [bold magenta]{s.total_dirs:>3}[/] directories, [bold magenta]{s.total_files:>3}[/] files"
        )
    else:
        print(f"\nSummary{size_str}:")
...
class TextRenderer(BaseRenderer):
    ...
    def _render_recursive(
        ...
        if self.config.show_size:
            size_str = (
                f"[{format_size_classic(node.size, self.config.human_readable)}] "
            )
        ...
        for i, child in enumerate(node.children):
            self._render_recursive(
                child, file, new_indent, i == len(node.children) - 1, False
            )
