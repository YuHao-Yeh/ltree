...
def test_get_icon_directories_emoji():
    ...
    assert (
        provider.get_icon("node_modules", is_dir=True)
        == f"{emoji.FOLDERS['node_modules']} "
    )

    # 2. unknown directories
    assert (
        provider.get_icon("my_custom_folder", is_dir=True) == f"{emoji.DEFAULT_FOLDER} "
    )
...
def test_get_icon_files_emoji():
    ...
    # 1. matched filename
    assert (
        provider.get_icon("Dockerfile", is_dir=False)
        == f"{emoji.FILENAMES['Dockerfile']} "
    )
    assert (
        provider.get_icon("README.md", is_dir=False)
        == f"{emoji.FILENAMES['README.md']} "
    )

    # 2. file extension
    assert provider.get_icon("script.py", is_dir=False) == f"{emoji.EXTENSIONS['.py']} "
    assert (
        provider.get_icon("style.css", is_dir=False) == f"{emoji.EXTENSIONS['.css']} "
    )

    # 3. uppercase file extensions
    assert (
        provider.get_icon("IMAGE.PNG", is_dir=False)
        == f"{emoji.EXTENSIONS.get('.png', emoji.DEFAULT_FILE)} "
    )
